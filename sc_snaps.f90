...
read(10,*) ntype
allocate(atname(ntype))
read(10,*) (natom0(j),j=1,ntype)
read(10,*) (ms(j),j=1,ntype),(ch(j),j=1,ntype)
read(10,*) (atname(j),j=1,ntype)
...
! write(ulog,*)"3 ",eival(3*natom-2)
 ...
 do i=4,3*natom
   if(eival(i).gt.-1d-5 ) then
      freq(i)=sqrt(abs(eival(i)))*cnst/33 ! to convert to 1/cm and then to THz
      write(ulog,*)i,freq(i)
!      modes(:,i)=eivec(:,i)
   else
